...
// import JobSeekerMessaging from '../components/JobSeekerMessaging';
...
//               borderRadius:12, padding:'10px 20px', width:'fit-content', marginTop:4,
//             }}>
//               <span style={{ fontSize:13, color:'rgba(200,215,255,0.85)', fontWeight:500 }}>
//                 🔓 Search jobs freely — log in to apply
//               </span>
//               <button onClick={() => navigate('/login')} style={{
//                 background:'white', color:'#1a2caa', border:'none',
...
//           {msgOpen && <JobSeekerMessaging user={user} onClose={() => setMsgOpen(false)} />}
...
const LandingPage = () => {
  ...
  useEffect(() => {
    if (user && (user.role?.toLowerCase() === 'recruiter' || user.role?.toLowerCase() === 'employer')) {
      navigate('/dashboard');
    }
  }, [user, navigate]);
  ...
  /* ════ RENDER ════ */
  return (
    <div style={{ minHeight:'100vh', fontFamily:"'Segoe UI',Tahoma,Geneva,Verdana,sans-serif", overflowX:'hidden' }}>
      <Navbar />
<div style={{ paddingTop:64 }}>
        {activeSection==='home' && (
          <HomeSection
